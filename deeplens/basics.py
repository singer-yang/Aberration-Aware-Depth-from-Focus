...
DEPTH = - 20000
GEO_SPP = 2048   # spp for geometric optics calculation
COHERENT_SPP = 10000000 # spp for coherent ray tracing

MINT = 1e-5
MAXT = 1e5
DELTA = 1e-6
EPSILON = 1e-9  # replace 0 with EPSILON
...
# Classes
# ===========================================
class DeepObj(nn.Module):  # be called when using print() and str()
    def __str__(self):
    ...
    def to(self, device=DEVICE):
        ...
        for key, val in vars(self).items():
            if torch.is_tensor(val):
                ...
                exec('self.{x}.to(device)'.format(x=key))
...
class Ray(DeepObj):
    def __init__(self, o, d, wvln=DEFAULT_WAVE, normalized=True, ra=None, en=None, obliq=None, opl=None, coherent=False, device=DEVICE):
        """ Ray class. A group of rays with the same wvln.
        ...
        """
        self.o = o if torch.is_tensor(o) else torch.tensor(o).type(torch.float32)
        self.d = d if torch.is_tensor(d) else torch.tensor(d).type(torch.float32)
        self.wvln = wvln if wvln < 10 else wvln * 1e-3 # store wvln in [um]
        ...
        self.obliq = obliq if obliq is not None else torch.full(o.shape[:-1], 1., dtype=torch.float32)  # used for obliquity energy decay
        self.opl = opl if opl is not None else torch.full(o.shape[:-1], 0., dtype=torch.float32)
        self.phi = torch.zeros_like(self.opl)
    ...
    def propagate_to(self, z, n=1):
        ...
        if self.coherent:
            ...
            k = 2 * np.pi / (self.wvln * 1e-3)    # k: [mm]^{-1}
            self.phi = self.phi + torch.fmod(n * k * (t - t.min(0).values), 2 * np.pi) 
    ...
    def ior(self, wvln):
        """ Compute the refractive index at given wvln. 
            ...
            Args:
                wvln (float): wvln
        """
        wv = wvln if wvln < 10 else wvln * 1e-3 # convert to [um]
